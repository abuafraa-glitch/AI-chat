...
class ContinuousLearningPipeline:
    ...
    # Semantic similarity is only a secondary guard after exact hash
    # deduplication.  A conservative threshold avoids collapsing distinct
    ...
    async def _stage_deduplication(self, run: PipelineRun, samples: List[DataSample]) -> List[DataSample]:
        ...
        if has_numpy and len(hash_unique) > 1:
            try:
                ...
                # embed() once per sample serializes model inference and made
                # ordinary learning runs appear to hang under regression.
                embeddings = await emb_manager.embed_texts_to_vectors(
                    [s.instruction[:512] for s in hash_unique]
                )
    ...
    async def _stage_fine_tuning(self, run: PipelineRun) -> None:
        ...
        # لا يُسمح لمسار الإنتاج بتنزيل نموذج تدريبي بعيد تلقائياً. التدريب
        # لا يبدأ إلا عند وجود checkpoint محلي كامل وقابل للقراءة.
        model_path = Path(base_model)
        has_local_checkpoint = model_path.is_dir() and (
            (model_path / "config.json").exists()
            and any((model_path / name).exists() for name in ("model.safetensors", "pytorch_model.bin"))
        )
        if not has_local_checkpoint:
            ...
            with open(deferred_path, "w", encoding="utf-8") as f:
                json.dump({
                    ...
                    "config": cfg,
                    "error": f"local training checkpoint is unavailable: {base_model}",
                    "queued_at": time.time(),
                    "status": "deferred_local_checkpoint_required",
            ...
            run.record_stage(PipelineStage.FINE_TUNING, 0, "مؤجل: checkpoint محلي مطلوب")
            return
    ...
    async def _stage_evaluation(self, run: PipelineRun) -> None:
        ...
        try:
            ...
            # Evaluation is production-authoritative and must never pull a
            # remote model implicitly.  A real checkpoint must already exist
            # on disk; otherwise the run remains non-deployable.
            model_path = output_dir or run.training_config.get("base_model", "Qwen/Qwen2.5-1.5B")
            model_path_obj = Path(model_path)
            if not model_path_obj.is_dir():
                raise FileNotFoundError(
                    f"local evaluation checkpoint is unavailable: {model_path}"
                )
            ...
            tokenizer = AutoTokenizer.from_pretrained(
                str(model_path_obj), trust_remote_code=True, local_files_only=True
            )
            if tokenizer.pad_token is None:
                tokenizer.pad_token = tokenizer.eos_token
            ...
            # Do not substitute a heuristic score for a missing/invalid real
            # model.  That would make an unavailable model look deployable.
            logger.warning("learning_pipeline: real evaluation unavailable: %s", e)
            results.update({
