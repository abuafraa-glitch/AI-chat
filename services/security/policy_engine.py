...
def tool_permission_policy(context: Dict[str, Any]) -> PolicyDecision:
    ...
    # Unknown tools fail closed for ordinary agents. Privileged agents are the
    # explicit authority boundary; no arbitrary tool is exposed to users.
    if agent_id not in {"admin_agent", "system_agent"}:
        return _deny(
            f"Agent '{agent_id}' is not authorized to use tool '{tool_name}'.",
            severity="high",
        )
    return _allow(f"Tool '{tool_name}' permitted for privileged agent '{agent_id}'.")
...
def content_moderation_policy(context: Dict[str, Any]) -> PolicyDecision:
    ...
    for phrase in HARMFUL:
        if phrase in text:
            return _deny(f"Content violates moderation guidelines: '{phrase}'", severity="critical")
...
class PolicyEngine:
    ...
    def enforce(self, context: Dict[str, Any]) -> bool:
        """Enforce all active policies and return a strict boolean decision."""
        results = self.evaluate(context)
        return all(bool(decision.get("allowed", False)) for decision in results.values())

    def enforce_or_raise(self, context: Dict[str, Any]) -> None:
        """Enforce all policies and raise PolicyViolationError if denied."""
        allowed = self.enforce(context)
        if not allowed:
            raise PolicyViolationError("Policy denied the action.")
