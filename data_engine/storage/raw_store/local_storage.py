...
class LocalRawStorage(BaseRawStorage):
    ...
    _BINARY_SUFFIXES = frozenset({
        ".bin", ".dat", ".bytes", ".gz", ".zip", ".tar", ".png", ".jpg",
        ".jpeg", ".gif", ".webp", ".pdf", ".mp3", ".mp4", ".wav", ".webm",
    })
    ...
    async def load_raw(self, key: str) -> Union[str, bytes]:
        ...
        # Preserve the caller's binary contract for known binary formats even
        # when their first bytes happen to be valid UTF-8.  Text formats retain
        # the convenient string return type; undecodable files remain bytes.
        if file_path.suffix.lower() in self._BINARY_SUFFIXES:
