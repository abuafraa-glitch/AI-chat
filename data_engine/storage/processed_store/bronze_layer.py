...
class BronzeLayer(BaseProcessedStorage):
    ...
    def _get_storage_path(self, key: str, version: Optional[str] = None) -> Path:
        ...
        if len(parts) == 1:
            # Public save_processed callers commonly pass only the item id;
            # use the validated schema name as the storage namespace.
            schema_name = "bronze"
            item_id = parts[0]
        elif len(parts) >= 2:
            schema_name = parts[0]
    ...
    async def save_processed(self, data: Dict[str, Any], key: str, schema_name: str, version: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        if schema_name == "BronzeSchema":
            # Backward-compatible adapter for the StorageManager public API:
            # older callers provide ``content`` while BronzeSchema uses the
            # canonical ``cleaned_content`` field.
            normalized_data = dict(data)
            if "cleaned_content" not in normalized_data and "content" in normalized_data:
                normalized_data["cleaned_content"] = normalized_data["content"]
            normalized_data.setdefault("raw_data_key", "")
            validated_data = BronzeSchema(**normalized_data).model_dump_json(indent=2)
        else:
    ...
    async def load_processed(self, key: str, schema_name: str, version: Optional[str] = None) -> Dict[str, Any]:
        ...
        """
        # Accept either the relative path returned by save_processed or the
        # original logical item id used by StorageManager.
        file_path = (
            self.base_dir / key
            if "/" in key
            else self._get_storage_path(key, version)
        )
