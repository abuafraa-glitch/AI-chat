...
class GoldLayer(BaseProcessedStorage):
    """Implementation of the Gold layer for processed data storage.
    This layer stores final, curated datasets ready for consumption.
    """

    def __init__(self, base_dir: Union[str, Path] = "./data/processed/gold") -> None:
    ...
    async def save_processed(self, data: Dict[str, Any], key: str, schema_name: str, version: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        Args:
            ...
            key (str): A unique identifier for the data (e.g., 'gold/report_456').
            schema_name (str): The name of the schema to use for validation (e.g., 'GoldSchema').
        ...
        """
        if schema_name == "GoldSchema":
            validated_data = GoldSchema(**data).model_dump_json(indent=2)
        else:
    ...
    async def load_processed(self, key: str, schema_name: str, version: Optional[str] = None) -> Dict[str, Any]:
        ...
        if schema_name == "GoldSchema":
            validated_data = GoldSchema(**data).model_dump()
        else:
            raise ValueError(f"Unsupported schema: {schema_name}")
    ...
    async def list_processed(self, schema_name: str, prefix: Optional[str] = None) -> AsyncIterator[str]:
        ...
        Args:
            schema_name (str): The name of the schema to filter by (e.g., 'GoldSchema').
