...
class PipelineOrchestrator(BasePipeline):
    ...
    async def run(
        ...
        # When the run starts without preloaded articles, FetchStage may
        # populate the context asynchronously.  In that case the effective
        # input is the fetch count, not the initial empty seed list.
        effective_input_count = input_count
        if effective_input_count == 0 and context.get("fetch_count") is not None:
            effective_input_count = int(context.get("fetch_count", 0))
