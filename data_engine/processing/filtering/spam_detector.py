...
def check_keyword_spam(
    ...
    matched = ""
    for kw in kws:
        # Match complete words/phrases only.  Substring counting would
        # incorrectly classify benign words such as ``scale`` as spam because
        # they contain a keyword such as ``sale``.
...
def check_repeated_phrases(
    ...
    # A whole article may legitimately contain a repeated block twice (for
    # example, syndicated or test content). Treat repetition as spam only
    # when the same trigram occurs at least three times, which preserves the
    # detector's ability to catch phrase-burst advertising.
    has_burst = max(counts.values(), default=0) >= 3
    return ratio > max_ratio and has_burst, round(ratio, 4)
